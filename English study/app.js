...
async function callGemini(prompt) {
  ...
  for (const model of modelsToTry) {
    const attempts = model === primaryModel ? 3 : 2;
...
function shouldUseGeminiFallback() {
  return fallbackToggle.checked;
}

function isRetryableGeminiError(error) {
  return error.status === 503 || error.status === 429 || error.message.includes("UNAVAILABLE");
}
...
function buildGeminiFinalError(errors) {
  ...
  if (hasRateLimit) {
    return new Error("Google Gemini 요청 한도에 걸렸습니다. 잠시 후 다시 시도하거나 Google API 사용량 한도를 확인해 주세요.");
  }
...
saveApiButton.addEventListener("click", () => {
  ...
  localStorage.setItem(STORAGE_KEYS.fallback, fallbackToggle.checked ? "true" : "false");
  apiKeyInput.value = "";
  updateStorageStatus();
...
fallbackToggle.checked = localStorage.getItem(STORAGE_KEYS.fallback) !== "false";
renderHistory();
